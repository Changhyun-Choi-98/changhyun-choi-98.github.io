...
{% for recent_page in recent_pages %}
  ...
  {% endif %}
- [{{ recent_page.title }}]({{ recent_page.url | relative_url }}){% if recent_page.tags contains "Korean" %} <span class="label label-blue home-post-label">Korean</span>{% endif %}{% if recent_page.tags contains "English" %} <span class="label label-purple home-post-label">English</span>{% endif %} <span class="label {{ recent_status_class }} home-post-label">{{ recent_status }}</span> <span class="text-grey-dk-000">{{ recent_page.date | date: "%Y-%m-%d" }}</span>{% if recent_page.summary %}<br>{{ recent_page.summary }}{% endif %}
  {% if recent_count == 5 %}
    {% break %}
  {% endif %}
{% endfor %}
