...
tags:
  - Korean
language: ko
summary: "CUDA와 Triton 전반에 대한 정리"
math: true
comments: true
...
OpenAI Triton은 CUDA 전체를 대체하는 것이 아니라, CUDA 스택 중에서 주로 **custom GPU kernel을 CUDA C++/NVCC로 직접 작성하는 부분에 대응되는 Python 기반 DSL(Domain-Specific Language)/compiler**이다. 따라서 Triton kernel도 결국 NVIDIA GPU에서 실행되려면 CUDA driver/runtime 계층과 GPU hardware 위에서 돈다.
...
중요한 점은 CUDA Runtime API는 CUDA Driver API 위에 구현되어 있다는 것이다. Runtime API는 대부분의 C++ CUDA 프로그램이 쓰는 간편한 API이고, Driver API는 context, module loading, lower-level control을 더 직접적으로 다룬다. 즉 Driver API는 CUDA context와 module 같은 더 낮은 개념을 노출한다.
...
[CUDA-accelerated Libraries]
  ...
  cuDNN                -> convolution, normalization, attention/graph-style DL ops
...
[GPU Hardware]
  ...
  copy engines, NVLink/PCIe, NVDEC/NVENC depending GPU
```
...
PTX는 여러 GPU generation을 포괄하는 안정적인 instruction set/programming model을 제공하고, high-level compiler가 PTX를 만든 뒤 target architecture native instruction으로 변환한다. PTX만 들어 있는 경우에는 runtime에 NVIDIA driver가 PTX를 native binary로 JIT compile할 수 있다. 이 binary code는 compute cache에 저장될 수 있다.
...
`nvidia-smi`가 작동한다는 것은 보통 driver와 GPU device discovery가 된다는 뜻이다. 하지만 `nvidia-smi`에 보이는 CUDA version은 설치된 CUDA Toolkit 버전이 아니라 현재 설치된 NVIDIA driver가 지원하는 최대 CUDA API/Runtime 호환 버전이다. 실제 개발 toolkit version은 `nvcc --version`, PyTorch build CUDA version(PyTorch binary가 포함/기대하는 CUDA 런타임 버전)은 `torch.version.cuda`로 따로 봐야 한다. 제대로 작동하기 위해서는 NVIDIA driver가 지원하는 CUDA version이 PyTorch/TensorRT가 요구하는 CUDA runtime version보다 같거나 이후의 버전이어야 한다.

### **CUDA Toolkit**
CUDA C++를 직접 컴파일하거나 native extension을 빌드하려면 CUDA Toolkit이 필요하다. CUDA Toolkit에는 보통 다음과 같은 것들이 들어있다:
...
/usr/local/cuda-<version>/
  ...
  include/
    cuda_runtime.h
    cublas_v2.h
    cudnn*.h if separately installed/integrated
    ...
  lib64/
    libcudart.so
    libcublas.so
...
## **OpenAI Triton은 CUDA의 어느 부분에 대응되는가?**
